...
def data_generator(file_path, batch_size, seq_len=512):
    while True:
        with xopen(file_path, "rt") as f:
            ...
            sublist = []
            batch_index = 0
            for i in range(get_label_dim(args.dev)):
                labels_list.append(np.zeros((batch_size,)))
            # print(labels_list)
            for line in cr:
                if len(text) == batch_size:
                    # Fun fact: the 2 inputs must be in a list, *not* a tuple. Why.
                    yield ([np.asarray(text), np.zeros_like(text)], labels_list)
                    text = []
                    batch_index = 0
                    # for i in range(len(labels_list)):
                    #     labels_list[i] = []
                text.append(np.asarray(json.loads(line[0]))[0:seq_len])
                for i, label in enumerate(json.loads(line[1])):
                    labels_list[i][batch_index] = label
                batch_index += 1
               
            # Yield what is left as the last batch when file has been read to its end.
            labels_list = [listerino[:batch_index] for listerino in labels_list]
            yield ([np.asarray(text), np.zeros_like(text)], labels_list)
...
class Metrics(Callback):
    ...
    def __init__(self):
        ...
        with xopen(file_name, "rt") as f:
            ...
            next(cr) # Skip example number row.
            for line in cr:
                self.all_labels.append([label for label in json.loads(line[1])])
                # if len(self.all_labels) == 32:
                #     break
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        labels_prob = np.transpose(np.squeeze(np.asarray(labels_prob)))
        ...
        for threshold in np.arange(args.threshold_start, args.threshold_end, args.threshold_step):
            ...
            #print(labels_prob[0], labels_pred, self.all_labels)
...
def build_model(args):
    ...
    attention_layers = []
    for i in range(get_label_dim(args.train)):
        self_attention = SeqSelfAttention(attention_width=27, attention_activation='sigmoid')(drop_mask)
        attention_layers.append(GlobalAveragePooling1D()(self_attention))

    output_layers = []
    for i in range(get_label_dim(args.train)):
        output_layers.append(Dense(1, activation='sigmoid')(attention_layers[i]))

    model = Model(bert.input, output_layers)
